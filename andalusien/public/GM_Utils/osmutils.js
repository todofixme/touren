...
JB.Map = function(makemap) {
	...
	const mycp = '<a href="https://www.j-berkemeier.de/GPXViewer" target="_blank" title="GPX Viewer '+JB.GPX2GM.ver+'">GPXViewer</a> <span aria-hidden="true">|</span> ';
	...
	this.baseLayers["Satellit"] = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
		maxZoom: 21,
		attribution: mycp+'&copy;&nbsp;<a href="https://www.esri.com" target="_blank">Esri</a>'
	...
	this.baseLayers["OSM"] = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
		maxZoom: 19,
		attribution: mycp+'&copy;&nbsp;<a href="https://www.openstreetmap.org/" target="_blank">OpenStreetMap</a> and contributors'
	});
	...
	this.baseLayers["Open Topo"] = L.tileLayer('https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png', {
		maxZoom: 17,
		attribution: mycp+'<br>©&nbsp;OpenStreetMap-Mitwirkende, SRTM |<br>© <a href="https://opentopomap.org/about" target="_blank">OpenTopoMap</a> (CC-BY-SA)'
	});
	...
	this.baseLayers["TopPlusOpen"] = L.tileLayer('https://sgx.geodatenzentrum.de/wmts_topplus_open/tile/1.0.0/web/default/WEBMERCATOR/{z}/{y}/{x}.png', {
		maxZoom: 18,
		attribution: mycp+'©&nbsp;<a href="https://www.bkg.bund.de/SharedDocs/Produktinformationen/BKG/DE/P-2017/170922-TopPlus-Web-Open.html" target="_blank">BKG</a>'
	});

	if(makemap.parameters.show_ign) {
		this.baseLayers["IGN Satellite"] = L.tileLayer("https://data.geopf.fr/wmts?" + "&REQUEST=GetTile&SERVICE=WMTS&VERSION=1.0.0" + "&STYLE=normal" + "&TILEMATRIXSET=PM" + "&FORMAT=image/jpeg" + "&LAYER=ORTHOIMAGERY.ORTHOPHOTOS" + "&TILEMATRIX={z}" + "&TILEROW={y}" + "&TILECOL={x}", {
			minZoom : 0,
			maxZoom : 18,
			attribution : mycp+"©&nbsp;IGN/Geoplateforme", tileSize : 256,
			errorTileUrl:JB.GPX2GM.Path+"Icons/Url-Fehler.png"
		}); 
		
		this.baseLayers["IGN Plan"] = L.tileLayer('https://data.geopf.fr/wmts?SERVICE=WMTS&VERSION=1.0.0&REQUEST=GetTile&STYLE=normal&TILEMATRIXSET=PM&FORMAT=image/png&LAYER=GEOGRAPHICALGRIDSYSTEMS.PLANIGNV2&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}', {
			maxNativeZoom: 18,
			maxZoom: 22,
			attribution: mycp+"©&nbsp;IGN/Geoplateforme",
			errorTileUrl:JB.GPX2GM.Path+"Icons/Url-Fehler.png"
		});
		
		this.baseLayers["IGN Topo"]= L.tileLayer('https://data.geopf.fr/private/wmts?service=WMTS&apikey=ign_scan_ws&request=GetTile&version=1.0.0&tilematrixset=PM&tilematrix={z}&tilecol={x}&tilerow={y}&layer=GEOGRAPHICALGRIDSYSTEMS.MAPS&format=image/jpeg&style=normal', {
			minZoom: 0,
			maxZoom: 18,
			attribution: mycp+"©&nbsp;IGN/Geoplateforme",
			errorTileUrl:JB.GPX2GM.Path+"Icons/Url-Fehler.png"
	...
	if(JB.GPX2GM.OSM_Cycle_Api_Key && JB.GPX2GM.OSM_Cycle_Api_Key.length>0) {
		this.baseLayers["Cycle"] = L.tileLayer('https://{s}.tile.thunderforest.com/cycle/{z}/{x}/{y}.png?apikey='+JB.GPX2GM.OSM_Cycle_Api_Key, {
			maxZoom: 22,
			attribution: mycp+'&copy;&nbsp;<a href="https://www.thunderforest.com/" target="_blank">OpenCycleMap</a> and contributors <a href="https://creativecommons.org/licenses/by-sa/2.0/" target="_blank">CC-BY-SA</a>'
		});
	}
	...
	if(JB.GPX2GM.OSM_Landscape_Api_Key && JB.GPX2GM.OSM_Landscape_Api_Key.length>0) {
		this.baseLayers["Landscape"] = L.tileLayer('https://{s}.tile.thunderforest.com/landscape/{z}/{x}/{y}.png?apikey='+JB.GPX2GM.OSM_Landscape_Api_Key, {
			maxZoom: 22,
			attribution: mycp+'&copy;&nbsp;<a href="https://www.thunderforest.com/" target="_blank">OpenLandscapeMap</a> and contributors <a href="https://creativecommons.org/licenses/by-sa/2.0/" target="_blank">CC-BY-SA</a>'
		});
	}
	...
	if(JB.GPX2GM.OSM_Outdoors_Api_Key && JB.GPX2GM.OSM_Outdoors_Api_Key.length>0) {
		this.baseLayers["Outdoors"] = L.tileLayer('https://{s}.tile.thunderforest.com/outdoors/{z}/{x}/{y}.png?apikey='+JB.GPX2GM.OSM_Outdoors_Api_Key, {
			maxZoom: 22,
			attribution: mycp+'&copy;&nbsp;<a href="https://www.thunderforest.com/" target="_blank">OpenOutdoorsMap</a> and contributors <a href="https://creativecommons.org/licenses/by-sa/2.0/" target="_blank">CC-BY-SA</a>'
		});
	}
...
JB.Map.prototype.change = function(maptype, overlay) { 
	if(this.makemap.parameters.maptype !="") maptype = this.makemap.parameters.maptype;
	else maptype = maptype || this.makemap.parameters.defaultmaptype;
	if(this.makemap.parameters.overlay !="") overlay = this.makemap.parameters.overlay;
	else overlay = overlay || maptype;
	maptype = maptype.toLowerCase().replace(/(^osm_)|(^osm )|(^osm-)/, "").replace(/( )|(_)|(-)/g, "");
	maptype = (maptype in this.layerNameTranslate) ? this.layerNameTranslate[maptype] : "OSMDE";
	maptype = (maptype in this.baseLayers) ? maptype : "OSMDE";
	overlay = overlay.toLowerCase().replace(/(^osm_)|(^osm )|(^osm-)/, "").replace(/( )|(_)|(-)/g, "");	
	overlay = (overlay in this.layerNameTranslate) ? this.layerNameTranslate[overlay] : "";
	if(maptype in this.baseLayers) {
	...
	if(this.currentOverlayLayer) this.currentOverlayLayer.remove();
	if(overlay in this.overlayLayers) {
		this.overlayLayers[overlay].addTo(this.map);
		this.currentOverlayLayer = this.overlayLayers[overlay];
	}
} // change
...
JB.Map.prototype.infowindow = function(info,coord) {
	var popup = L.popup({maxWidth: this.map.getContainer().offsetWidth*0.8, 
											 maxHeight: this.map.getContainer().offsetHeight*0.9, 
											 autoClose: false }) 
		.setLatLng(coord);
