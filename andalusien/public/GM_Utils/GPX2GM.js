...
JB.GPX2GM.ver = "6.22";
JB.GPX2GM.dat = "16. 9. 2025";
JB.GPX2GM.fname = "GPX2GM.js";
JB.GPX2GM.globalMapParameter = {};

(function() {
	JB.GPX2GM.Path = "";
	JB.GPX2GM.autoload = true;
	const scr = document.getElementsByTagName("script");
	let urlObject,pars;
	for(let i=scr.length-1;i>=0;i--) if(scr[i].src && scr[i].src.length) {
		let path = scr[i].src;
		let pos = path.search(JB.GPX2GM.fname);
		if(pos!=-1) {
			JB.GPX2GM.Path = path.substring(0,pos);
			path = path.replace(/;/g,"&");
			pars = (new URL(path)).searchParams;
			JB.GPX2GM.scriptSearchParameter = {};
			pars.forEach((value, key) => { JB.GPX2GM.scriptSearchParameter[key.toLowerCase()] = value; });
			if(pars.has("autoload", false)) JB.GPX2GM.autoload = false;
			break;
		}
	}
	pars = (new URL(location.href.replace(/;/g,"&"))).searchParams;
	JB.GPX2GM.searchParameter = {};
	pars.forEach((value, key) => { JB.GPX2GM.searchParameter[key.toLowerCase()] = value; });
	if(typeof(GPXVIEW_Debuginfo)=="undefined") 
		JB.debuginfo = pars.has("debuginfo") && !pars.has("debuginfo", false) ;    
	else
	...
	this.ShowGPX = function(fn,mpt) {
		...
		JB.lpgpx.call(this,file,id,function(daten) {
			...
			if (this.parameters.legende) {
				...
				mapdiv.style.height = `calc(100% - ${t}px)`; 
			}
			show.call(this);
...
JB.loadFile = function(file, format, callback) {
	if(!file.fileobject) { // ajax
		JB.loadFile_fetch(file, format, callback);
	}
	else { //File API
...
JB.loadFile_fetch = function(file, format, callback) {
	const id = "loadFile_xml";
	const url=file.name;
	let result={asciidata:"<gpx></gpx>"};
	if(url.length==0) {
	...
	if(fetch) {
		JB.Debug_Info(id,"fetch");
		let lastmodified, lmstr;
		fetch(url, {mode: 'no-cors'} )
			.then(function(response) {
				JB.Debug_Info(id,"Datei konnte geladen werden, Status: "+response.status+", Datei: "+url, false);
				...
				if(format != "b") response.text()
					.then(function(text) {
						result.asciidata = text;
						callback(result, response.status, lmstr);
					} )
					.catch(function(error) {
						JB.Debug_Info(id,"Fehler "+error+" bei Ascii-Datei "+url, true);					
						callback(result,response.status,null);
					} );
				else response.arrayBuffer()
					.then(function(buffer) {
						result.binarydata = new Uint8Array(buffer); // ??? ungetestet ???
						callback(result, response.status, lastmodified);
					} )
					.catch(function(error) {
						JB.Debug_Info(id,"Fehler "+error+" bei Binary-Datei "+url, true);					
						callback(result,response.status,null);
					} );
			})
			.catch(function(error) {
				JB.Debug_Info(id,"Fetch liefert Fehler "+error+" bei Datei "+url, true);
				callback(result,-1,null);
			} );
	}
	else {
		JB.Debug_Info(id,"fetch wird nicht unterstützt, Datei: "+url, true);
		callback(result,-1,null);
	} 	
}; // loadFile_fetch
...
JB.GPX2GM.start = function() {
	...
	JB.LoadScript(JB.GPX2GM.Path+"GPX2GM_Defs.js", function() { 
		...
		for(let par in JB.GPX2GM.scriptSearchParameter) {
			if(par in JB.GPX2GM.parameters) {
				if(JB.GPX2GM.scriptSearchParameter[par] == "false") JB.GPX2GM.parameters[par] = false;
				else if(JB.GPX2GM.scriptSearchParameter[par] == "true") JB.GPX2GM.parameters[par] = true;
				else JB.GPX2GM.parameters[par] = JB.GPX2GM.scriptSearchParameter[par];
			}
			else {
				JB.Debug_Info("start","Unbekannter Parameter: " + par,false);
				console.error("Unbekannter Parameter: " + par);
			}
		}
	...
	if(typeof(Bestaetigung)=="undefined" || Bestaetigung) {
		...
		if(localStorage) {
			...
			if(ls && ls.length) {
				...
				var frage   = (JB.GPX2GM.parameters.mapapi=="gm")? 
						JB.GPX2GM.strings[doc_lang].frage_datenschutz_gm : 
				    JB.GPX2GM.strings[doc_lang].frage_datenschutz_osm ;
				var antwort = (JB.GPX2GM.parameters.mapapi=="gm")? 
						JB.GPX2GM.strings[doc_lang].antwort_datenschutz_gm : 
...
JB.GPX2GM.check_API = function() {
	...
	const mapapi_par = JB.GPX2GM.searchParameter.mapapi || JB.GPX2GM.searchParameter.map_api;
	if(mapapi_par) {
		switch(mapapi_par) { 
			case "osm":
				mapapi = "osm"; 
